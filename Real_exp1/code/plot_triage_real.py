...
class plot_triage_real:
    ...
    def plot_err_vs_K(self, image_file, plot_obj, file_name, test_method, idx):
        ...
        for option in plot_obj.keys():
            plt.plot(plot_obj[option][key], label=key + ' ' + option, linewidth=8, linestyle='--', marker='o',
                     markersize=10)
        plt.grid()
        plt.legend()
        plt.xlabel('K')
            ...
            for K, i1 in zip(self.list_of_K, range(len(self.list_of_K))):
                for lamb, i2 in zip(self.list_of_lamb, range(len(self.list_of_lamb))):
                    ...
                    if option in res[str(std)][str(K)][str(lamb)]:
                        ...
                        res[str(std)][str(K)][str(lamb)][option]['test_res'] = {'ranking': test_res_r,
...
def main():
    ...
    # specify std, lamb and filename as specified in ReadMe.txt
